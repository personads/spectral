...
for seed in "${SEEDS[@]}"; do
  ...
  for task in "${TASKS[@]}"; do
    ...
    for lang in "${LANGUAGES[@]}"; do
      # convert data
      if [ ! -f $DATA_PATH/$lang-train-$task.csv ]; then
        echo "Converting $lang to CSV format..."
        python tasks/amazon/convert.py $DATA_PATH
      else
        echo "Using existing CSV data at '$DATA_PATH/$lang-(train|dev)-$task.csv'."
      fi

      # setup experiment root for TB
      exp_root="$EXP_PATH/$task/$lang/$LM_SHORT"
      if [ ! -f $exp_root ]; then
        mkdir -p "$exp_root"
      fi
      ...
      train "$exp_dir" "$filter" "$DATA_PATH/$lang-train-$task.csv" "$DATA_PATH/$lang-dev-$task.csv" $seed
      evaluate "$exp_dir" "$filter" "$DATA_PATH/$lang-train-$task.csv" "$DATA_PATH/$lang-dev-$task.csv" $seed

      # train without filter
      exp_dir="$exp_root/nofilter-rs$seed"
      train "$exp_dir" "nofilter()" "$DATA_PATH/$lang-train-$task.csv" "$DATA_PATH/$lang-dev-$task.csv" $seed
      evaluate "$exp_dir" "nofilter()" "$DATA_PATH/$lang-train-$task.csv" "$DATA_PATH/$lang-dev-$task.csv" $seed
    done
  done
done
